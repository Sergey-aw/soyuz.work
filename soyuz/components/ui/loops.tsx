...
const formStyles = {
  ...
  "formFont": "Forma-Regular",
  "formFontColor": "#000000",
  "formFontSizePx": 16,
  "buttonText": "Напишите, когда откроетесь",
  "buttonFont": "Forma-Regular",
  "buttonFontColor": "#ffffff",
  "buttonColor": "#000000",
  "buttonFontSizePx": 16,
  "successMessage": "Спасибо! Сообщим вам как только будут новости.",
  "successFont": "Forma-Regular",
...
export default function LoopsForm() {
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!isValidEmail(email)) {
      setFormState(ERROR);
      setErrorMessage("Кажется это не похоже на email");
      return;
    }
    ...
    fetch(`https://${domain}/api/newsletter-form/${formStyles.id}`, {
      ...
      .catch((error) => {
        ...
        if (error.message === "Failed to fetch") {
          setErrorMessage("Слишком много попыток, подождите немного");
  ...
  function SignUpFormError() {
    return (
      ...
      >
        ...
        >
          {errorMessage || "Что-то не сработало, попробуйте ещё разок."}
        </p>
      </div>
    );
  }
  ...
  function SignUpFormButton({ props }: any) {
    return (
      ...
      >
        {formState === SUBMITTING ? "Добавляем вас..." : formStyles.buttonText}
      </button>
    );
  }
}
